...
try:
    ...
    for chunk in pd.read_csv(output_file_path, chunksize=chunk_size):
        ...
        connection_point_sum = chunk.filter(like="connection_point_exp_injection").sum(axis=1)
        ...
        grid_tariff_low = chunk.filter(like="grid_tariff_low_exp_out").sum(axis=1)
        grid_tariff_high = chunk.filter(like="grid_tariff_high_exp_out").sum(axis=1)
        ...
        chunk.insert(18, "grid_tariff_low", grid_tariff_low)
        chunk.insert(19, "grid_tariff_high", grid_tariff_high)
...
for i in range(1, 201):  # Adjust range as needed
    try:
        ...
        if os.path.exists(file_path):
            ...
            grid_cost_low = single_values["grid_cost_low"].iloc[0]
            grid_cost_high = single_values["grid_cost_high"].iloc[0]

            # Append the data to their respective lists
            objective_values.append({"household": i, "objective_value": objective_value})
            grid_costs.append({"household": i, "grid_cost_low": grid_cost_low, "grid_cost_high": grid_cost_high})
...
if grid_costs:  # Ensure there are values to save
    ...
    total_grid_cost_low = combined_grid_cost_df["grid_cost_low"].sum()
    total_grid_cost_high = combined_grid_cost_df["grid_cost_high"].sum()
    total_grid_cost = total_grid_cost_low + total_grid_cost_high
    combined_grid_cost_df = pd.concat(
        [combined_grid_cost_df, pd.DataFrame([{"household": "Total", "grid_cost": total_grid_cost, "grid_cost_low": total_grid_cost_low, "grid_cost_high": total_grid_cost_high}])],
        ignore_index=True
    )
