# uv run 1_extraction_cap_sub.py
import pandas as pd
import iesopt
import os
...
for i in range(start_index, 201):  # Ends with 201
    try:
        ...
        timeseries_data = results.to_pandas(
            filter=lambda c, t, f: (
                c in [
                    ...
                    "heat_unit", "water_unit", "grid_tariff_low", "grid_tariff_high"
                ]
                and t == "exp"
            ),
            orientation="wide"
        ...
        grid_cost_low = results.components["grid_tariff_low"].obj.cost  # Fetch the grid cost
        grid_cost_high = results.components["grid_tariff_high"].obj.cost  # Fetch the grid cost

        # Create a single DataFrame containing both values
        single_values_df = pd.DataFrame({
            "objective_value": [objective_value],
            "grid_cost_low": [grid_cost_low],
            "grid_cost_high": [grid_cost_high]
        })
...
# file_name = f"results/cap_sub/household_11.iesopt.result.jld2"
# results = iesopt.Results(file=file_name)

# # Convert to pandas DataFrame and save as CSV
# df_household = results.to_pandas(orientation='wide') 
# df_household.to_csv('results/cap_sub/a_household_11.csv', index=False)
