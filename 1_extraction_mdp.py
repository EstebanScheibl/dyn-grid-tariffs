...
start_index = 155 # <----- start with 1
end_index = 201 # 201

# Loop through household files from start_index to 200
for i in range(start_index, end_index):  # Ends with 201
    try:
        ...
        print(f"Processed single values for household {i} successfully")
...
# file_name = f"results/base_model/household_130.iesopt.result.jld2"
# results = iesopt.Results(file=file_name)

# # Convert to pandas DataFrame and save as CSV
# df_household = results.to_pandas(orientation='wide') 
# df_household.to_csv('results/base_model/a_household_130.csv', index=False)
