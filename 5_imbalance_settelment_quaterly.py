...
for model_name, path in models.items():
    ...
    net_load_lv_grid_df['season'] = net_load_lv_grid_df['month'].apply(get_season)

    # Group data by season
    seasonal_dfs = {season: net_load_lv_grid_df[net_load_lv_grid_df['season'] == season] for season in ["Winter", "Spring", "Summer", "Autumn"]}

    # Function to filter data by day type for each season
    def separate_by_day_type(season_df):
        return {
            "Weekday": season_df[season_df['day_type'] == 'Weekday']['grid_load'],
            "Saturday": season_df[season_df['day_type'] == 'Saturday']['grid_load'],
            "Sunday": season_df[season_df['day_type'] == 'Sunday']['grid_load']
        }

    # Split seasonal data by day type
    seasonal_data = {season: separate_by_day_type(df) for season, df in seasonal_dfs.items()}

    # Function to calculate representative daily profile
    def calculate_representative_day(data):
        daily_data = data.values.reshape(-1, 96)  # Reshapes into 96 intervals per day
        return pd.DataFrame(daily_data.mean(axis=0), columns=['grid_load'])

    # Compute representative days for each season and day type
    representative_days = {}
    for season, day_types in seasonal_data.items():
        representative_days[season] = {}
        for day_type, data in day_types.items():
            if not data.empty:  # Check if data exists for that season and day type
                representative_days[season][day_type] = calculate_representative_day(data)
            else:
                print(f"Warning: No data for {day_type} in {season}")
                representative_days[season][day_type] = pd.DataFrame([0] * 96, columns=['grid_load'])

    # Construct full-year schedule based on seasonal patterns
    ...
    for i, row in net_load_lv_grid_df.iterrows():
        ...
        full_year_schedule.at[i, "scheduled_load"] = representative_days[season][day_type].iloc[time_of_day_idx].values[0]
