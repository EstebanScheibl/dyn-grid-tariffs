# uv run 4_combined_result_handling_cap_sub.py
...
models = {
    ...
    "Cap. sub. 7.5kW": "results/cap_sub_ext/run_200hh/combined_household_data.csv",
    "Cap. sub. 4kW": "results/cap_sub/run_200hh/combined_household_data.csv"

}
otherfiles = {
    "summary": "files/household_summary.csv"}

output_file_path = "results/a_combined_results_cap_sub/"
...
model_colors = {
    ...
    "Cap. sub. 7.5kW": ("brown", "solid"),
    "Cap. sub. 4kW": ("magenta", "solid")
...
for model_name, df in dataframes.items():
    if 'net_load_lv_grid' in df.columns:
        ...
        fig_ldc.add_trace(go.Scatter(
            ...
            mode='lines',
            name=model_name,
            line=dict(color=color, dash=line_style, width=0.8)  # Apply colors & dash styles dynamically
        ))
...
fig_ldc.add_annotation(
    text="①: Peak load<br>②: Intermediate<br>      load<br>③: Base load",
    xref="paper", yref="paper",
    x=1.4, y=0.2,  # Positioned towards center, slightly lower
    showarrow=False,
    align="left",
...
# uv run 4_combined_result_handling.py
#****************************************************************************************************************
# grid cost extraction for cap_sub 
cap_sub_grid_df = pd.read_csv(r"C:\Users\ScheiblS\Documents\Repositories\dyn-grid-tariffs\results\cap_sub\run_200hh\combined_grid_cost.csv")
cap_sub_data = cap_sub_grid_df["grid_cost_low"].iloc[:200] + cap_sub_grid_df["grid_cost_high"].iloc[:200] + 57.6  # Begrenzung auf 200 Werte
cap_sub_model_df = dataframes["Cap. sub. 4kW"]

# Erstelle eine Liste für den Verbrauch der Haushalte
household_consumption_cap_sub = []

for i in range(1, 201):  # Von HH1 bis HH200
    consumption = cap_sub_model_df[f"el_market_exp_value_{i}"].sum() / 4 if f"el_market_exp_value_{i}" in cap_sub_model_df else 0
    household_consumption_cap_sub.append(consumption)
...
# Füge die einzelnen Farbgruppen hinzu, damit sie in der Legende erscheinen
fig.add_trace(go.Scatter(
    x=household_consumption_cap_sub[:20],  # Erste 20 Haushalte (FCG1)
    y=cap_sub_data[:20],
    mode='markers',
    marker=dict(color='blue', size=5),
    name="FCG1"  # Legendenname
...
fig.add_trace(go.Scatter(
    x=household_consumption_cap_sub[20:50],  # 21-50 Haushalte (FCG2)
    y=cap_sub_data[20:50],
    mode='markers',
    marker=dict(color='green', size=5),
    name="FCG2"  # Legendenname
))
...
# Füge die restlichen Farbgruppen hinzu
fig.add_trace(go.Scatter(x=household_consumption_cap_sub[50:80], y=cap_sub_data[50:80], mode='markers',
                         marker=dict(color='purple', size=5), name="FCG3"))
fig.add_trace(go.Scatter(x=household_consumption_cap_sub[80:110], y=cap_sub_data[80:110], mode='markers',
                         marker=dict(color='orange', size=5), name="FCG4"))
fig.add_trace(go.Scatter(x=household_consumption_cap_sub[110:160], y=cap_sub_data[110:160], mode='markers',
                         marker=dict(color='cyan', size=5), name="FCG5"))
fig.add_trace(go.Scatter(x=household_consumption_cap_sub[160:200], y=cap_sub_data[160:200], mode='markers',
                         marker=dict(color='magenta', size=5), name="IFCG"))
...
# Speichern der Dateien
pdf_file_path = "C:\\Users\\ScheiblS\\Documents\\Repositories\\dyn-grid-tariffs\\results\\a_combined_results_cap_sub\\a_scatter_grid_cost_cap_sub.pdf"
fig.write_image(pdf_file_path, format='pdf')
print(f"Scatterplot has been saved as 'a_scatter_grid_cost_cap_sub.pdf'")

html_file_path = "C:\\Users\\ScheiblS\\Documents\\Repositories\\dyn-grid-tariffs\\results\\a_combined_results_cap_sub\\a_scatter_grid_cost_cap_sub.html"
fig.write_html(html_file_path)
print(f"Scatterplot has been saved as 'a_scatter_grid_cost_cap_sub.html'")

# uv run 4_combined_result_handling.py
#****************************************************************************************************************
# grid cost extraction for cap_sub_ext
cap_sub_ext_grid_df = pd.read_csv(r"C:\Users\ScheiblS\Documents\Repositories\dyn-grid-tariffs\results\cap_sub_ext\run_200hh\combined_grid_cost.csv")
cap_sub_ext_data = cap_sub_ext_grid_df["grid_cost_low"].iloc[:200] + cap_sub_ext_grid_df["grid_cost_high"].iloc[:200] + 57.6  # Begrenzung auf 200 Werte
cap_sub_ext_model_df = dataframes["Cap. sub. 4kW"]

# Erstelle eine Liste für den Verbrauch der Haushalte
household_consumption_cap_sub_ext = []

for i in range(1, 201):  # Von HH1 bis HH200
    consumption = cap_sub_ext_model_df[f"el_market_exp_value_{i}"].sum() / 4 if f"el_market_exp_value_{i}" in cap_sub_ext_model_df else 0
    household_consumption_cap_sub_ext.append(consumption)
...
# Füge die einzelnen Farbgruppen hinzu, damit sie in der Legende erscheinen
fig.add_trace(go.Scatter(
    x=household_consumption_cap_sub_ext[:20],  # Erste 20 Haushalte (FCG1)
    y=cap_sub_ext_data[:20],
    mode='markers',
    marker=dict(color='blue', size=5),
    name="FCG1"  # Legendenname
))
...
fig.add_trace(go.Scatter(
    x=household_consumption_cap_sub_ext[20:50],  # 21-50 Haushalte (FCG2)
    y=cap_sub_ext_data[20:50],
    mode='markers',
    marker=dict(color='green', size=5),
    name="FCG2"  # Legendenname
))
...
# Füge die restlichen Farbgruppen hinzu
fig.add_trace(go.Scatter(x=household_consumption_cap_sub_ext[50:80], y=cap_sub_ext_data[50:80], mode='markers',
                         marker=dict(color='purple', size=5), name="FCG3"))
fig.add_trace(go.Scatter(x=household_consumption_cap_sub_ext[80:110], y=cap_sub_ext_data[80:110], mode='markers',
                         marker=dict(color='orange', size=5), name="FCG4"))
fig.add_trace(go.Scatter(x=household_consumption_cap_sub_ext[110:160], y=cap_sub_ext_data[110:160], mode='markers',
                         marker=dict(color='cyan', size=5), name="FCG5"))
fig.add_trace(go.Scatter(x=household_consumption_cap_sub_ext[160:200], y=cap_sub_ext_data[160:200], mode='markers',
                         marker=dict(color='magenta', size=5), name="IFCG"))
...
# Speichern der Dateien
pdf_file_path = "C:\\Users\\ScheiblS\\Documents\\Repositories\\dyn-grid-tariffs\\results\\a_combined_results_cap_sub\\a_scatter_grid_cost_cap_sub_ext.pdf"
fig.write_image(pdf_file_path, format='pdf')
print(f"Scatterplot has been saved as 'a_scatter_grid_cost_cap_sub_ext.pdf'")

html_file_path = "C:\\Users\\ScheiblS\\Documents\\Repositories\\dyn-grid-tariffs\\results\\a_combined_results_cap_sub\\a_scatter_grid_cost_cap_sub_ext.html"
fig.write_html(html_file_path)
print(f"Scatterplot has been saved as 'a_scatter_grid_cost_cap_sub_ext.html'")
...
fig.add_trace(go.Scatter(
    x=household_consumption_cap_sub,
    y=cap_sub_data,  # Handle NaN values
    mode='markers',
    marker=dict(color='brown', size=5),
    name="Cap. sub. 7.5kW"  # Adjusted name for clarity
))

fig.add_trace(go.Scatter(
    x=household_consumption_cap_sub_ext,
    y=cap_sub_ext_data,
    mode='markers',
    marker=dict(color='magenta', size=5),
    name="Cap. sub. 4kW"  # Adjusted name for clarity
))
...
pdf_file_path = "C:\\Users\\ScheiblS\\Documents\\Repositories\\dyn-grid-tariffs\\results\\a_combined_results_cap_sub\\a_combined_scatter_grid_cost.pdf"
fig.write_image(pdf_file_path, format='pdf')
print(f"Scatterplot has been saved as '{pdf_file_path}'")

html_file_path = "C:\\Users\\ScheiblS\\Documents\\Repositories\\dyn-grid-tariffs\\results\\a_combined_results_cap_sub\\a_combined_scatter_grid_cost.html"
fig.write_html(html_file_path)
