...
pv_min = start_index #start_index  # Minimum index for individual net load calculation = 1
...
try:
    ...
    for chunk in pd.read_csv(output_file_path, chunksize=chunk_size):
        ...
        ev_unit = chunk.filter(like="ev_unit_exp_out_electricity").sum(axis=1)
...
# Define the path for the combined objective value file
combined_objective_file = f"{csv_folder_path}/combined_objective_value.csv"

# List to hold aggregated objective values
objective_values = []

# Loop through the single_values_{i}.csv files
for i in range(start_index, end_index):  # Adjust range as needed
    try:
...
if not objective_values:
    print("No data was processed. No combined file was created.")
# uv run 2_fusion_mpd.py
# *************************************************************************************************